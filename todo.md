...
- [x] 导航新增「设置」项，点击展开下拉框（暂含「切换浅色/深色模式」）
...
### 2.7 关于页面 ✅
- [x] 关于页占位
- [x] 第一部分：姓名、职位、个人介绍、详细描述（Mock：about.json）
- [x] 第二部分：工作经历列表（时间、职位·公司、描述、技术栈标签）、查看完整简历链接占位（Mock：experience.json）
- [ ] 简历下载链接替换为真实文件（当前 resumeDownloadUrl 占位为 #）
- [ ] 完善联系方式等（可选）
...
- [ ] 实现文章收藏功能
- [ ] 实现评论系统（如果需要）
- [x] 实现暗黑模式切换（设置下拉 → 切换浅色/深色模式，Zustand + persist 持久化）
- [ ] 实现字体大小调整
